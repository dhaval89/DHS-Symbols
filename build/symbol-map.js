...
export var getSymbolMap = JSON.stringify(symbolMap);
/**
...
export var isSymbolMapValid = validateSymbolMap();
/**
 ...
 */
export function getFrame(sidc) {
    var returnFrame;
    if (sidc && sidc.length > 0 && symbolMap.catIdLength) {
        var index = 0;
        var parentCat = void 0;
        // Get the category id
 ...
 */
export function findFrameDefinition(frameName) {
    var frameDef;
 ...
 */
export function findAssetName(name, prefix) {
    var assetName = (prefix ? prefix : '') + name;
    return Object.keys(SVGAssets).find(function (key) {
