...
exports.Symbol = exports.SVGAssetMap = void 0;
...
var _fontkit = _interopRequireDefault(require("fontkit"));
...
var _a = require('svgdom'),
    createSVGWindow = _a.createSVGWindow,
    config = _a.config;

var window = createSVGWindow();
(0, _svg.registerWindow)(window, window.document);
var SVGAssetMap = _svgAssets.SVGAssets;
/**
 * Set up the fonts
...
exports.SVGAssetMap = SVGAssetMap;
...
fonts['Work Sans'] = _fontkit["default"].create(buffer);
buffer = fs.readFileSync('./fonts/WorkSans/WorkSans-Bold.ttf');
fonts['Work Sans Bold'] = _fontkit["default"].create(buffer);
var noFill = '#FFFFFF00';
var defaultViewBoxWidth = 100;
var defaultViewBoxHeight = 100;
var defaultAnchor = {
  x: defaultViewBoxWidth * 0.5,
  y: defaultViewBoxHeight * 0.5
};
var defaultDecoratorRadius = 10;
var defaultDecoratorPadding = 4;
var defaultDecoratorOffsetY = defaultDecoratorRadius + defaultDecoratorPadding; // Decorator offset maps
...
/** @class */
function () {
  function _Symbol2(sidc, style, badge, decorators) {
    var _this = this;

    if (sidc) {
      this.sidc = sidc;
      this.frame = (0, _symbolMap.getFrame)(sidc);
    } else {
      this.sidc = '';
    } // Set the defaults


    this.style = _style.styleDefaults;
    ...
    if (badge) {
      ...
      this.badge = __assign(__assign({}, _badge.badgeDefaults), badge);
    }

    if (decorators && decorators.length > 0) {
      this.decorators = [];
      decorators.forEach(function (partDec) {
        var _a; // Merge the arguments that were set with the defaults


        (_a = _this.decorators) === null || _a === void 0 ? void 0 : _a.push(__assign(__assign({}, _decorator.decoratorDefaults), partDec));
      });
    }

    this.width = defaultViewBoxWidth;
    this.height = defaultViewBoxHeight; // The anchor point for the current symbol
  ...
  _Symbol2.prototype.asSVG = function () {
    var _this = this;

    var svgContainer = (0, _svg.SVG)().namespace().viewbox(0, 0, defaultViewBoxWidth, defaultViewBoxHeight); // Create a root group

    var svgRootGroup = svgContainer.group().id('root'); //TODO: Use SVG.Color
    ...
    if (this.style.frame) {
      ...
      if (this.style.frameName) {
        ...
        this.frame = (0, _symbolMap.findAssetName)(this.style.frameName, 'frame-');

        if (!this.frame) {
          // Didn't find the frame asset, check custom frames
          this.frame = (0, _symbolMap.findFrameDefinition)(this.style.frameName);
        }
      }

      if (this.frame) {
        var frameGroup_1 = svgRootGroup.group().id('frame'); // Has a frame
        ...
        if (Object.prototype.hasOwnProperty.call(this.frame, 'framePath')) {
          ...
          if (frameSvg) {
            ...
            var frameContainer = (0, _svg.SVG)(frameSvg); //TODO: May need to fix this when viewbox adjustments are implemented
            // Update the primary viewbox based on the imported frame's viewbox

            svgContainer.viewbox(frameContainer.viewbox()); // Move any children from the temporary SVG into the root group of the SVG Container

            frameContainer.children().forEach(function (c) {
              if (c instanceof _svg.Shape) {
        ...
        paths.forEach(function (p) {
          if (p instanceof _svg.Path) {
            ...
            p.attr({
              'stroke-width': _this.style.frameWidth
            });
          }
        });
        /**
    ...
    if (this.style.icon) {
      var svgIcon = (0, _symbolMap.findAssetName)(this.sidc, 'icon-');

      if (svgIcon) {
        // Create a group for this icon
        var iconGroup_1 = svgRootGroup.group().id('icon'); // Retrieve the svg string

        var iconSvg = SVGAssetMap[svgIcon];

        if (iconSvg) {
          // Create the svg element
          var iconContainer = (0, _svg.SVG)(iconSvg); // Move all the svg children to the main svg
          ...
          iconContainer.children().forEach(function (c) {
            if (c instanceof _svg.Shape) {
              ...
              if (!hasStrokeWidth) {
                ...
                c.attr({
                  'stroke-width': _this.style.iconStrokeWidth
                });
    ...
    if (this.badge) {
      ...
      badgeGroup.circle(diameter).cx(this.badge.badgeRadius).cy(this.badge.badgeRadius).fill(this.badge.badgeFillColor).stroke(this.badge.badgeTextColor);
      badgeGroup.plain(count).font({
      ...
      .dx(this.badge.badgeRadius).dy(this.badge.badgeRadius + this.badge.badgeRadius * this.badge.badgeBaselineAdjust).fill(this.badge.badgeTextColor);
    ...
    if (this.decorators && this.decorators.length > 0) {
      ...
      var eloc = dloc;
      var decoratorLocation = _style.DecoratorLocation[eloc];
      var decSlots = void 0;

      if (decoratorLocation === _style.DecoratorLocation.BOTH) {
        // both
        decSlots = 6;
      } else if (decoratorLocation === _style.DecoratorLocation.TOP) {
        // top
        decSlots = 3;
      } else {
        // bottom
      ...
      var j_1 = 0;
      var k_1 = decoratorLocation === _style.DecoratorLocation.BOTTOM ? 1 : 0;
      this.decorators.forEach(function (d) {
        if (decCount_1 > j_1) {
          // break doesn't work the same in typescript
          var decAssetName = (0, _symbolMap.findAssetName)(d.name, 'dec-');
          ...
          if (decAssetName) {
            ...
            if (decAsset) {
              ...
              if (d.frame) {
                ...
                var circle = decGroup_1.circle(diameter_1).cx(decX[i_1][j_1]).cy(decY[k_1]).stroke(d.monoColor || d.frameColor);

                if (d.fill) {
                  circle.fill(d.fillColor).attr({
              ...
              var decContainer = (0, _svg.SVG)(decAsset);
              ...
              innerDecGroup_1.stroke(d.monoColor || d.iconColor).fill(d.monoColor || d.iconColor).transform({
    ...
    var paddingDouble = this.style.padding * 2; // Viewbox
    ...
    var h = vbox.height + paddingDouble + (this.style.decoratorMaxCount > 0 ? defaultDecoratorRadius * 4 + defaultDecoratorPadding * 4 : 0); // Size
    ...
    if (this.style.outline) {
      ...
      svgRootGroup.circle(diameter).id('outline').center(defaultAnchor.x, defaultAnchor.y).stroke(this.style.outlineColor).fill(noFill).attr({
        'stroke-width': this.style.outlineWidth
      });
    ...
    if (this.style.name && this.style.name.length > 0) {
      var nameText = (0, _svg.SVG)().plain(this.style.name).font({
      ...
      var nameBg = nameText.clone().attr('stroke', this.style.nameBgColor).attr('stroke-width', '0.25em'); // Now set it back to the base so that clients can use the weight attribute appropriately
      ...
      nameText.fill(this.style.nameColor); // Create a group for the labels

      var labelGroup = svgRootGroup.group().id('name');
      var dx = defaultViewBoxWidth + this.style.frameWidth * 0.5 + this.style.padding;
      labelGroup.add(nameBg).add(nameText).dx(dx).dy(defaultAnchor.y + textY * 0.25); // you'd think this adjustment would be 0.5
    ...
    this.width = sizeX;
    this.height = sizeY; // add center and dimension formatted as <desc>ctr:x=76,y=76:dim:x=152,y=152</desc>

    var descString = 'ctr:x=' + this.symbolAnchor.x + ',y=' + (this.symbolAnchor.y - this.style.padding) + ':dim:x=' + sizeX + ',y=' + sizeY;
